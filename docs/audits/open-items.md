...
| 6 | **`hack/ci.sh` runs neither `staticcheck` nor `deadcode`, and its `go test` has no `-race`** — while `docs/GO-STANDARDS.md` says three times that CI adds `-race`. Two lines would have caught items 3, and the deadcode set, at landing time. | Findings that a gate should catch arriving by audit instead | CONFIRMED |
...
| 9 | **`writeParents`/temp-file leak on rename failure** — 6 sites, inert today (no stale-temp consumption anywhere), and `kinemigrate.go` already cleans up at `:223/:254`, so the real argument is same-file inconsistency. | Cosmetic | Severity REFUTED |
