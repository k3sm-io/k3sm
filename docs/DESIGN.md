...
- **Services:** **userspace Service proxy** owning each ClusterIP:port (lo0 alias), watching Services/EndpointSlices; L4 LB to local lo0 IPs or remote pods over the mesh; NodePort binds `*:port`. Cap/batch lo0 aliases (watch the `getifaddrs`/configd O(N) cost at scale). kube-proxy never built.
...
- **DNS:** CoreDNS on a cluster VIP; pods resolve via a **`DYLD_INSERT_LIBRARIES` `getaddrinfo` shim** → CoreDNS (NOT resolv.conf; macOS uses mDNSResponder/configd).
...
- **Packaging/signing:** ship as an **app-bundle-wrapped root LaunchDaemon** (a bare daemon Mach-O can't carry entitlements/profiles). Code-running entitlements (`allow-jit`, `allow-unsigned-executable-memory`, `disable-library-validation`) are free and notarize fine; **stay on raw root utun/pf — no restricted NE/System-Extension capability** (avoids Apple approval risk). `k3sm install/uninstall` via `launchctl bootstrap/bootout/kickstart`; admin kubeconfig → `~/.kube/config`. User-folder access (if ever needed) requires pre-granted FDA via MDM PPPC.
...
- **M0 — Walking skeleton.** Build apiserver+scheduler+CM+kine from source as goroutines → `kubectl get nodes` Ready (VK node). Runtime in **HostProcess mode** (no isolation): provider `posix_spawn`s one native arm64 binary at host path; logs/exec/status/top via the provider. **Exit:** `kubectl run` a native workload → it executes; zero Linux end-to-end. *Prototype-first (highest risk): a native Foundation-linked binary runs under a default-deny Seatbelt profile, seeing `/System`, confined to its pod dir — no chroot.*
- **M1 — `k3sm server` + images + Services + DNS (single node).** Single binary (Embedded executor, KCM scoped, admission policy, kine pinned); OCI pull → `clonefile` pod dir → ad-hoc-sign → Seatbelt confine; userspace Service proxy + CoreDNS + **`getaddrinfo` DNS shim** (prototype early). **Exit:** pull a native image, run it, `kubectl expose` ClusterIP, DNS resolves.
- **M2 — Isolation + resources + daemon split.** SBPL generation (exec→inproc + CI symbol-canary); split `k3sm-runtimed` (root, gRPC); userspace memory kill (`OOMKilled`) + QoS + Summary API; IP-per-pod (lo0 alias + bind discipline). **Exit:** pods confined (no `/Users`), memory breach → `OOMKilled`, `kubectl top` real, same-node pod-to-pod.
- **M3 — Multi-node join + mesh.** `k3sm token create`; `k3sm agent --server --token` (node-password + HTTP-CSR); wireguard-go mesh + `MeshPeer` CRD; cross-node pod-to-pod + ClusterIP. *Prototype: two real Macs, wg utun, `iperf3` both directions, bounce a node → reconverge.* **Exit:** two Macs, one cluster, A↔B works.
- **M4 — Install/launchd, packaging, Homebrew, HA, hardening.** App-bundle-wrapped root LaunchDaemon + `k3sm install/uninstall`; codesign+notarize (code-running entitlements; raw utun/pf, no NE); `.pkg`; **goreleaser release pipeline publishing the `k3sm-io/homebrew-tap` Formula**; kine→Postgres HA; `uidjail` backend, probes, NodePort/LB; CI on macOS arm64; watch-staleness soak; Darwin-subset node conformance. **Exit:** `brew install k3sm-io/tap/k3sm` → `sudo k3sm install server` → running cluster; survives reboot; HA; core repos public.
- **M5 (stretch) — `vm` RuntimeClass** (Virtualization.framework) for untrusted/strong-isolation tenants.
...
- M4: `brew install` → `sudo k3sm install` → cluster; reboot → auto-starts (launchd); `.pkg` install/uninstall clean; HA with 2 servers on Postgres; watch-staleness soak green.
- Track a documented **subset of upstream conformance**; full Sonobuoy is a stretch goal.
