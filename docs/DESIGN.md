...
>
> **Status: v2 (2026-06-24), red-teamed.** A 4-persona adversarial pressure-test against a macOS 26.5.1 machine + primary sources reshaped the runtime and networking designs (see §3). This is the living design doc; the four repos live under `/Users/operator/Code/k3sm-io/`.
...
- **DNS:** a per-node resolver on the cluster DNS VIP; pods resolve via a **`DYLD_INSERT_LIBRARIES` `getaddrinfo` shim** → that VIP (NOT resolv.conf; macOS uses mDNSResponder/configd). **M3 ships an in-process A-record + upstream-forward resolver** (IPv4-only; no SRV/PTR/headless), **not** CoreDNS-the-binary — darwin-net exposes no embeddable DNS server and CoreDNS-the-binary cannot inherit the netd-helper-passed `<1024` socket fd under launchd (mainline CoreDNS has no socket activation), whereas the in-process resolver *can* adopt that fd. The VIP's `<1024` bind + lo0 alias go through `k3sm-netd`; the proxy steps aside for the DNS VIP (infra-VIP exemption) so the resolver owns it. **Follow-up — real CoreDNS as a native `darwin/arm64` binary:** CoreDNS is pure Go, so it builds native (the way the executor builds kine) and run supervised on the VIP would give full SRV/PTR/headless natively; deferred behind the unprivileged-posture fd-bind blocker above (in root mode it could bind the VIP, but the production posture is unprivileged and a root-only resolver would be a second DNS path). The missing SRV/PTR/headless records are **moot on the native hostprocess path** (every pod reports `podIP == nodeIP`, so there are no per-pod/headless addresses) and become **material only on the per-pod-IP `vm` path** — where the native-CoreDNS follow-up pays off.
...
- **RBAC enforcement (M4.1):** the apiserver flips `AlwaysAllow → Node,RBAC` + the `NodeRestriction` admission plugin. It is a **pure authorizer switch** (no two-phase restart): `pkg/rbac.Provision` lays down the graph at server start — **before** the VK node and the worker-join supervisor — **fail-closed** (a provisioning error halts bring-up; a half-applied graph under an enforcing authorizer silently locks workers out), running under the retained `system:masters` admin client (RBAC-exempt). The **node-datapath ClusterRole** (bound to the `system:nodes` group) grants a joined worker read on `services` / `endpointslices` / `meshpeers` — the Node authorizer and the stock `system:node` role grant **none** of these, so without it a worker's Service proxy + DNS resolver + mesh watcher (`system:node:<name>`) would be denied and the datapath would die at the flip. **Documented component-identity divergence (adopted, NOT expanded):** RBAC is enforced for **workloads + joined-worker `system:node` identities**; the in-process VK node / scheduler / KCM / provisioners / enroller **retain `system:masters`** (the static admin token). Moving them to per-component client certs is a **deferred** follow-up — it would break the pure-switch property (boot would need component certs provisioned + a node re-bootstrap) for no M4.1 benefit, since they run in the one trusted `_k3sm` process. `NodeRestriction` is **core-resource-only** (Node/Pod own-object writes); it never covers the `net.k3sm.io/MeshPeer` CRD, so the MeshPeer write-guard (`bootstrap.AuthorizeMeshPeerWrite`) is **permanent** (the node gets `meshpeers` READ via the ClusterRole; the WRITE stays server-mediated).
...
## 6. Repositories (cloned at `/Users/operator/Code/k3sm-io/`)
...
**Workspace wire-up:**
```sh
cd /Users/operator/Code/k3sm-io
...
- **M5 (committed) — `vm` RuntimeClass** (Virtualization.framework Linux micro-VM), behind the existing swappable `sandbox.Backend` seam. Two purposes: strong isolation for untrusted tenants **and** running **Linux** images with no native arm64 build — the HYBRID path for stockkitty's Postgres/pgvector (see `../../docs/stockkitty-readiness.md`). `runtimeClassName: vm` maps to the existing `SANDBOX_BACKEND_VM` (the upstream `node.k8s.io/RuntimeClass` is consumed, not forked); guest images are digest-pinned (codesign/notarization is meaningless inside the VM); networking is vmnet/bridged with a guest-side resolver. **Dispatch + scheduling gate (M5.1 foundation, built):** the provider resolves `spec.runtimeClassName` through the apis handler-config (`runtimev1.DefaultHandlerConfig().Backend`) and stamps `SandboxProfile.Backend` (+ `VmVcpus`/`VmMemoryBytes` guest sizing for a `vm` pod) — empty → `UNSPECIFIED` so runtimed's `SelectBackend` picks the host-OS-gated rung (the host-process default is *never* a hardcoded Seatbelt rung), unknown handler → fail closed. Placement is fail-closed via a `node.k8s.io/v1` RuntimeClass `vm` whose `scheduling.nodeSelector` requires the `k3sm.io/virtualization` node label, set only on a VZ-capable node; absent label ⇒ a `vm` pod stays `Unschedulable`. **Open:** the node VZ label needs a per-backend availability signal — runtimed's `GetRuntimeInfo` currently reports only the *selected* backend's health, so a `GetRuntimeInfo` extension is required to set the label truthfully (until then it defaults absent). **Exit:** a Linux image runs under `runtimeClassName: vm`, Service/DNS-reachable, beside native pods.
