...
- **M4 — Install/launchd, packaging, Homebrew, HA, hardening.** App-bundle-wrapped root LaunchDaemon + `k3sm install/uninstall`; codesign+notarize (code-running entitlements; raw utun/pf, no NE); `.pkg`; **goreleaser release pipeline publishing the `k3sm-io/homebrew-tap` Formula**; kine→Postgres HA; `uidjail` backend; **RBAC enforcement** (`AlwaysAllow → Node,RBAC`, with the system roles + VK-node `system:node` identity + NodeRestriction bootstrap provisioned *before* the flip); LoadBalancer; CI on macOS arm64; watch-staleness soak; the **synthetic conformance gate** green in CI + Darwin-subset node conformance. **Exit:** `brew install k3sm-io/tap/k3sm` → `sudo k3sm install server` → running cluster; survives reboot; HA; core repos public.
