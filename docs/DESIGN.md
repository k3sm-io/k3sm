...
- **Services:** **userspace Service proxy** owning each ClusterIP:port (lo0 alias), watching Services/EndpointSlices; L4 LB to local lo0 IPs or remote pods over the mesh; NodePort is bound by `k3sm-netd` on the **specific node address** (never wildcard `*`), the port authorized against the Service set (default 30000–32767; a `<1024` bind only if a Service declares it). Cap/batch lo0 aliases (watch the `getifaddrs`/configd O(N) cost at scale). kube-proxy never built.
