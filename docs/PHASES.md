...
phases:
  ...
  - id: M4
    ...
    status: in-progress
    note: "M4.1 (RBAC enforcement) is CODE-COMPLETE + unit-proven: the apiserver default is now --authorization-mode=Node,RBAC + NodeRestriction (pkg/executor), and pkg/rbac.Provision lays down the node-datapath ClusterRole (system:nodes ⇒ read services/endpointslices/meshpeers) + the in-pod reader RoleBinding, FAIL-CLOSED before the node/join-supervisor start. The LIVE authz flip is the build-tagged e2e/TestM4_RBACEnforced, RUN GREEN on the lab host 2026-08-27 (M4.1-a1 met:true) and re-run green 2026-09-25. M4.2 (synthetic conformance gate) is DONE: the live integration legs ran green on the dev Mac 2026-09-25 (m2.sh 10/10 with all 13 required criteria PASS none skipped; single-node m3.sh 2/2), flipping M4.2-a1 met:true. M4.0 rides M7.1 by phases_ref (its acceptance flips only via the M7.1 write-through), so M4 stays open on that row alone; the two-Mac hack/lab/m3.sh rung ran GREEN 2026-09-25 on a live laptop-server + studio-worker rig (NodePort, PVC persistence, in-pod kubectl + cluster DNS on the joined worker, all over the wireguard mesh), so the remaining M4-adjacent validation is the B28 churn soak alone (the M7.0 burn-down stub)."
    depends_on:
      - apis:M4.1
      - runtimed:M4
      - darwin-net:M4
