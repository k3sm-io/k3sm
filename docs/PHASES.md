  ...
  - id: M11
    ...
    subphases:
      - id: M11.0
        ...
        status: in-progress  # 2026-08-31 — d1 (S1) done; d5 (S5) no-root slice recorded; d2 deferred per the arm64-only slice; d3 blocked on a virtiofs=y kernel; d4 open
        ...
        deliverables:
          ...
          - id: M11.0-d3  # S3 virtiofs semantics + perf
            done: false  # 2026-08-31: CANNOT START on the throwaway Ubuntu kernel — it builds virtio_fs as a MODULE and the guest has no module tree, so the share cannot mount (observed: no such device). This empirically confirms d4's built-in-only requirement; S3 waits on a kernel with the virtiofs stack =y
