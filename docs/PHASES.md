  ...
  - id: M8
    ...
    subphases:
      ...
      - id: M8.6
        ...
        acceptance:
          - id: M8.6-a1
            met: false  # lab-ledger carve-out, FINAL STATE 2026-08-30 after four agent-driven lab runs: every product defect between the milestone and this gate is FIXED AND VERIFIED LIVE (argv-in-rootfs, workingDir, pod TMPDIR, image ls, kubelet port, served-model-id — runtimed#81/#82, k3sm#202/#203/#204); the image half is proven (byte-identical determinism, walk-verify 353/353, digest 1852da7d…); the serving stack is proven under Seatbelt by counterfactual (Ready 1/1, Metal, 4/4 concurrent continuous batching, 255-272 tok/s); rootless stops at the DOCUMENTED mount-resolution ceiling (limitations.md — the path shim is root-tier). THE ONE REMAINING STEP: the root-tier m8.sh run (operator sudo session on the apple-gpu rig; runbook = hack/images/mlx-serve/README.md publish-or-import, then K3SM_MLX_IMAGE=@sha256 m8.sh). Rig logs: ~/k3sm-spike-m8/logs/.
      ...
      - id: M11.4
        ...
        deliverables:
          ...
          - id: M11.4-d5
            ...
            desc: "Admission + docs: vm pods still carry nodeSelector kubernetes.io/os=darwin (the node IS darwin; the guest is an implementation detail) — the chart incantation (os=darwin + runtimeClassName: vm) documented in the reference-workload readiness notes (internal) + limitations.md. The foreign-user VAP exemption (scope foreignUserExpr by runtimeClassName==vm; security-engineer-authored CEL; native pods keep the full pin) is HUMAN-GATED B112 — tracked, never built by a wave; MERGE-PRECONDITIONS (re-sequencing, plan R21): only after the S3(2) idmap answer records YES and the volume/fsGroup mechanism deliverable has landed; its gate asserts the triple (vm+fsGroup admitted / native+fsGroup still 422 / vm-on-non-VZ-node fails closed) + update-or-recreate semantics for the stored VAP (DISCHARGED by B153: every pkg/policy Ensure* is now create-or-update in place). USER-DOC COPY FLIP (owned here, v0.1 framing — the launch pre-flight denylist blocks the stale strings): faq.md's Linux-images answer, vm-runtimeclass.md's not-launch-blocking line, limitations.md's native-images-only + v0.2-headlines lines, the public ROADMAP Shipped/Non-goals bullets; the rewrite states what EXPERIMENTAL promises (the m11-core-proven behaviors incl. PVC durability) and denies (figures pending; vm-surface changes allowed until the flip; the NETWORK-AXIS ceiling: same-node vm pods share one NAT segment NetworkPolicy cannot segment; the vmhost confinement posture per the S1(4) outcome) — mirrored into the privilege-model doc. Rosetta posture: directory share only, no cross-VM translation-cache socket at v0.1."
        acceptance:
          - id: M11.4-a1
            met: false
