      ...
      - id: M11.0
        ...
        deliverables:
          ...
          - id: M11.0-d3  # S3 virtiofs semantics + perf
            done: false  # 2026-08-31: CANNOT START on the throwaway Ubuntu kernel — it builds virtio_fs as a MODULE and the guest has no module tree, so the share cannot mount (observed: no such device). This empirically confirms d4's built-in-only requirement; S3 waits on a kernel with the virtiofs stack =y. 2026-08-31 (same day, LATER): UNBLOCKED — s3-run.sh swaps in a >=6.12 throwaway that SHIPS the module (Ubuntu 25.04 plucky 6.14.0-37-generic + the exact matching linux-modules .deb; fuse is =y there and virtiofs.ko declares no deps, so one finit_module before the mount suffices), and the gate WAS measured: S3(2)=NO — mount_setattr(MOUNT_ATTR_IDMAP) on the virtiofs mount returns EINVAL(22) while a tmpfs control and a MOUNT_ATTR_NOSUID on the SAME virtiofs tree both succeed, so the refusal is the filesystem instance, not the helper/userns/syscall. Consequence per s3.sh: human-reviewed fsGroup re-plan, NO chmod fallback pre-encoded, and the CORE storage leg substitutes a root-in-guest image with PVC-PGDATA-with-fsGroup moving to the follow-on; B112 does NOT unblock. Also recorded: fsync on the share median 422us over N=50 (tmpfs 12us), emptyDir IO 1237 MiB/s seq write and 31 MiB/s / 7935 IOPS random-4K (tmpfs 9724 / 2721 MiB/s), and APFS case-collision passes THROUGH virtiofs in BOTH directions with a SUCCESS return and one readdir entry (silent loss; detection must be host-side). STILL NOT RUN, so d3 stays open: S3(1)'s pgbench 0.25x bar (no database in the throwaway guest — NEITHER the un-park NOR the delete branch fires and the virtio-blk hatch stays parked), the guest-fsync->host-fsync(2)-vs-F_FULLFSYNC half (needs host-side tracing, needs root), S3(3) the ownership sidecar (needs the product unpacker tree), S3(6) confined-vs-unconfined throughput. Findings: hack/spike/m11/findings-s3.md; the gate answer is macOS-26.6.2-build-specific and should be re-measured against the shipping macOS floor before B112 is decided either way
          ...
          - id: M11.0-d5  # S5 guest networking
            done: false  # 2026-08-31 PARTIAL — every no-root criterion is recorded in findings-s5.md as a decision table: (6) the host observes the GUEST'S OWN vmnet IP, no gateway rewrite => per-pod attribution is a straight map and the lease->pod registry is buildable; (5) lease stable across restarts under a deterministic MAC; (7) MTU 1500 unclamped (clamp is a DHCP/init-plan action); (2) host->guest round trip works; (4) guest<->guest UNREACHABLE both protocols (ARP never resolves peer, gateway resolves — inverts the assumed trust ceiling; one-rig undocumented behaviour, not publishable as a ceiling); (1a) a loopback-bound host listener receives NOTHING from the guest; (1b) a guest-side route is accepted and packets LEAVE the guest. REMAINING = the root-needing human slice: the real lo0-alias VIP arrangements (a)-(d) and criterion 3 (PodIP-as-eth0-alias + host route) — the "(b) alone suffices" branch is deliberately NOT yet selected on guest-half evidence alone
