  ...
  - id: M14
    ...
    subphases:
      ...
      - id: M14.4
        ...
        deliverables:
          - id: M14.4-d1
            done: false  # 2026-09-10 — HALF discharged. The gate half is GREEN: K3SM_LAB=1 hack/lab/m3.sh exit 0 on the two-Mac rig at merged main bb63128, 3 passed / 0 failed, reproduced three times; both Macs ran the byte-identical binary (sha256 0ad847a2d18f…, so no skew), the mesh was verified directly rather than inferred (server.go:715-717 is log-and-continue), and the NodePort dial genuinely crossed the tunnel (the dial targeted the joined worker while the backend pod ran on the server node). Evidence run id: m3-lab-local-2026-09-10.log (the maintainers' run log). The SECOND half — site/content/tutorials/07-two-macs.md executed start to finish as written and recorded — was NOT attempted, so d1 stays false. That tutorial path is untested, not disproven; the unusable `install agent ...` invocation is in hack/lab/m3.sh's own header comment, not in the tutorial, which uses `sudo k3sm install` then a separate `k3sm agent ...`. 2026-09-17 UPDATE: the gate half re-greened on the CURRENT build — both Macs on the identical installed build 0.1.6-lab.56963a3, the worker joined with no hand-applied configuration, m3.0 PASS and all three TestM3_* PASS from the control-plane Mac, M3 gate exit 0 (run id parked-lab-2026-09-17.log, runs 16-17). The tutorial half is STILL not attempted, so d1 stays false.
        ...
        acceptance:
          - id: M14.4-a1
            met: false  # 2026-09-17 — FIRST CLAUSE DISCHARGED TWICE, second clause not dischargeable as written. Clause one, K3SM_LAB=1 hack/lab/m3.sh exit 0 on two real Macs with no hand-applied shims: green on 2026-09-01 (run id m3-lab-twomac-2026-09-01.log, run 6 — M3: 3 passed, 0 failed) and again on 2026-09-17 on the current build (run id parked-lab-2026-09-17.log, run 17, recorded in the maintainers' run log) — an M1 Max control-plane Mac and an M2 laptop worker, both on the identical installed build 0.1.6-lab.56963a3 (staged from k3sm main 56963a3; the gate driven from the control-plane Mac): m3.0 PASS (2 Ready nodes, worker identified), TestM3_NodePort PASS (1.95s), TestM3_PVCPersistsAcrossRestart PASS (2.04s), TestM3_InPodKubectlAndDNSOnWorker PASS (2.03s), m3.A PASS; the worker joined by `k3sm install --agent` and NO CRD, label, certificate or wireguard configuration was applied by hand at any point. Clause two is undischarged for a reason that is NOT a missing run: site/content/tutorials/07-two-macs.md is STALE against the shipped binary — it starts the control plane by hand with `k3sm server --mesh-ip <address>` where the installer now owns that address, and joins with `sudo k3sm install` followed by a hand-run `k3sm agent --server … --node-ip … --token <token>` where the shipped form is `sudo k3sm install --agent --server … --node-ip … --token-file …`. Executing it EXACTLY AS WRITTEN would not run, so the tutorial has to be rewritten before it can be executed and recorded; that rewrite is tracked in the maintainers' queue and M14.4-d1 keeps this half.
