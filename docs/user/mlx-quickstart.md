...
> **Requirements:** Apple Silicon (arm64), macOS 26+, a running k3sm cluster ([quickstart.md](quickstart.md)),
...
| `cache.storageClassName` | k3sm marks no StorageClass as default, so `local-path` must be named or the cache volume never binds. See [storage.md](storage.md). |
...
**retained** — the downloaded weights stay on disk until you remove them by hand ([storage.md](storage.md)).

## Things to know

- MLX serving runs on the **default native runtime path**. Do not set `runtimeClassName: vm`: the
  serving process needs direct access to the Mac's GPU, and the `vm` RuntimeClass
  ([vm-runtimeclass.md](vm-runtimeclass.md)) isolates a workload from exactly that.
