...
trust model — why this is the same privileged-helper pattern the desktop container tools and Linux
VM managers use, applied to k3sm, and the one residual limitation (no per-pod uid isolation) — is documented in
