...
An optional [`vm` RuntimeClass](vm-runtimeclass.md) runs `linux/arm64` Pods in a per-Pod micro-VM to
isolate untrusted workloads. See [Limitations](limitations.md).
...
short version is [What runs](what-runs.md); the reference is [Images](images.md); see also
...
[Limitations](limitations.md#restartpolicy-is-honored-on-the-default-runtime-not-on-the-hostprocess-opt-out).
