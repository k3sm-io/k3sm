...
func TestDevProvisionsDNSShim(t *testing.T) {
	...
	t.Run("both staged shims reach the server as --path-shim / --dns-shim", func(t *testing.T) {
		...
		args := serverArgs("dev", "/w", "/pods", 7443, "direct", runtimeRuntimed, wantPath, wantDNS)
	...
	t.Run("an unstaged shim emits no flag at all", func(t *testing.T) {
		args := serverArgs("dev", "/w", "/pods", 7443, "none", runtimeRuntimed, "", "")
		...
		args = serverArgs("dev", "/w", "/pods", 7443, "none", runtimeRuntimed, "/Library/k3sm-dev/"+pathShimName, "")
