...
#
# The REAL M10 gate (per docs/m10-plan.md §"Gate machinery", Res.9) is a COMPOSITE:
# its non-skeleton form execs the M10 conformance criteria so /orchestrate proves M10
# via its own canonical gate — composing M10.0's §-gate enforcement e2e (apply a
...
# exit 0 as "milestone proven", so a placeholder that exited 0 would fake-green M10.
# This gate is pinned always-RED by TestNonManualSkeletonsAlwaysRed.
set -euo pipefail
