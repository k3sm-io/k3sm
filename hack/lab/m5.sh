...
# resolves (hack/acceptance/phases_test.go) and so /orchestrate has a runnable
# referent under K3SM_LAB=1, not so it can prove the milestone.
#
...
#     VZ-capable rig, so it prints a PENDING notice and exits 0 — a "skip", which
...
#     be read by /orchestrate as "M5 proven" and fake-green the milestone; the gate
...
echo "         (under K3SM_LAB=1 this exits 1 by design so /orchestrate never fake-greens M5 from a placeholder; repo root: $REPO_ROOT)" >&2
exit 1
