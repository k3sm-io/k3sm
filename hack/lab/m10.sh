...
# M10.1 (docs/m10-plan.md §M10.1) and is NOT performed here. This file exists so the
# manifest's gate path resolves (hack/acceptance/phases_test.go) and so /orchestrate
# has a runnable referent under K3SM_LAB=1, not so it can prove the milestone.
#
...
#     multi-node rig, so it prints a PENDING notice and exits 0 — a "skip", which
...
#     An exit 0 here would be read by /orchestrate as "M10-lab proven" and fake-green
#     the milestone; the gate therefore stays RED until M10.1 replaces this skeleton
...
echo "         (under K3SM_LAB=1 this exits 1 by design so /orchestrate never fake-greens M10-lab from a placeholder; repo root: $REPO_ROOT)" >&2
exit 1
